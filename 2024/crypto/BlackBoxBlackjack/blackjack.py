...
class Blackjack:
    ...
    def show_hands(self, hand_index):
        name = self.player_names[hand_index -
                                 1] if hand_index > 0 else "Dealer"
    ...
    def show_final_winner(self):
        ...
        winners = [name for name, money in zip(
            self.player_names, self.player_moneys) if money == max_money]
        if len(winners) == 1:
